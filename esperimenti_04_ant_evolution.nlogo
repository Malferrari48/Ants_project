...
turtles-own [
  ...
  scorta
  metabolismo
  sgl_scorta  ; cibo min. per fare figli
]
...
to setup-turtles
  ...
  set angolo_virata random-float (angolo_virata_massimo - 10)
...
to cerca-cibo
  let psx senti_feromone_ad_angolo -45
  let pc senti_feromone_ad_angolo 0
  let pdx senti_feromone_ad_angolo 45
  let pmax max(list psx pc pdx)
...
to torna-nido
  let psx senti_nido_ad_angolo -45
  let pc senti_nido_ad_angolo 0
  let pdx senti_nido_ad_angolo 45
  let pmax max(list psx pc pdx)

  ; si va diritto, a meno che il segnale non sia più forte di lato
...
to try-copulate
  if any? other turtles-here [
    if [scorta] of item 0 sort-on [scorta] other turtles-here > sgl_scorta [
      ...
      ask patch-here [
        sprout 1 [
          setup-turtles
          set metabolismo one-of (list met_padre met_madre)
          set velocita one-of (list vel_padre vel_madre)
          set angolo_virata ((one-of (list ang_padre ang_madre)) * ((random-float 0.1) + 0.95))
          set n_nati n_nati + 1
        ]
      ]
      set scorta scorta - (scorta_tartarughe / 10)
    ]
  ]
end
...
to step
  ...
  ;Ricarica le fonti di cibo ogni 70 passi
  let resto ticks
...
true
false
"set-plot-x-range 10 angolo_virata_massimo\nset-plot-y-range 0 numero_termiti" ""
PENS
"default" 2.0 1 -16777216 true "" "histogram [angolo_virata] of turtles"
...
"" ""
PENS
"default" 1.0 1 -16777216 true "" "plot n_nati"
...
"pen-1" 1.0 1 -16777216 true "" "plot n_morti"
